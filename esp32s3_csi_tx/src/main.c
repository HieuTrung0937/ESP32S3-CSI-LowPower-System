...
ESP_LOGI(TAG, "TX ready - Controlled low-rate mode for CSI");
    ...
    while (1) {
        ...
        // Tiến hành gửi 1 gói tin duy nhất
        esp_now_send(MAC_BOARD_RX, (uint8_t *)&tx_data, sizeof(tx_data));

        // Bật nhanh LED để báo hiệu vừa có gói đi
        led_strip_set_pixel(led_strip, 0, 10, 0, 0);
        led_strip_refresh(led_strip);
        
        // Delay ngắn để mắt người nhìn thấy LED sáng
        vTaskDelay(pdMS_TO_TICKS(30)); 
        
        // Tắt LED để chuẩn bị cho chu kỳ sau
        led_strip_clear(led_strip);
        led_strip_refresh(led_strip);

        // KHỐNG CHẾ TỐC ĐỘ GỬI TẠI ĐÂY:
        // Đổi thành 1000 để nghỉ đúng 1 giây giữa các lần gửi (Tốc độ ~1 Hz)
        // Nếu muốn nhanh hơn một chút (2 gói/giây), bạn hãy đổi thành 500
