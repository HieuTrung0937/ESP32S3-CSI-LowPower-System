...
#define NEOPIXEL_PIN       GPIO_NUM_45  // THỬ ĐỔI NẾU KHÔNG SÁNG
...
#endif
{
    ESP_LOGI("CB", "Send callback #%lu, status=%d", send_count, status);
    
    if (status == ESP_NOW_SEND_SUCCESS) {
        ESP_LOGI(TAG, "Gui thanh cong");
        led_strip_set_pixel(led_strip, 0, 10, 0, 0);  // Xanh
        led_strip_refresh(led_strip);
    } else {
        ESP_LOGE(TAG, "Gui THAT BAI");
        led_strip_set_pixel(led_strip, 0, 0, 10, 0);  // Đỏ
        led_strip_refresh(led_strip);
    }
}
...
static void init_neopixel(void) {
    ...
    // TEST: Bật đỏ 500ms để xác nhận LED hoạt động
...
void app_main(void) {
    ...
    init_neopixel();    // LED sẽ nháy đỏ 500ms nếu OK
    init_wifi_espnow();
    ...
    while (1) {
        ...
        esp_err_t err = esp_now_send(MAC_BOARD_RX, (uint8_t *)&tx_data, sizeof(tx_data));
        if (err != ESP_OK) {
