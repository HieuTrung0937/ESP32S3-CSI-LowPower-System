...
static void IRAM_ATTR promiscuous_cb(void *buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (memcmp(mac, MAC_BOARD_TX, 6) == 0) {
        ...
        // CÁCH TIẾP CẬN AN TOÀN: Đọc trực tiếp thông tin từ điều khiển gói nhận
        // Nếu phần cứng tự động đính kèm thông tin core mang, ctrl->csi_len sẽ lớn hơn 0
        if (ctrl->sig_len > 0) {
            // Định dạng để Python nhận diện: CSI_RAW, RSSI, Độ dài CSI, payload...
            printf("CSI_RAW,%d,%d\n", ctrl->rssi, ctrl->sig_len);
        } else {
            // Nếu csi_len bằng 0, ta in RSSI tạm thời để Python vẽ trước dạng sóng cơ bản
            // Định dạng: RSSI_DATA, Số thứ tự, RSSI
